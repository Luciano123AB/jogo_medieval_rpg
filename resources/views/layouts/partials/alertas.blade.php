...
@if($alerta && !session()->has($chaveLancado))
    <script>
        Swal.fire({
            ...
            title: "<label class='d-grid gap-3 py-2'><span class='titulos_{{ $tema }} cor_fontes_{{ $tema }} {{ session('tema') == 'escuro' ? 'border-primary' : 'border-danger' }} border-top border-bottom py-3'>{{ session('alerta.titulo') }}</span><span class='cor_fontes_{{ $tema }} fs-5'><i class='cursor bi {{ session('alerta.icone') }}'></i>{{ $icone }} {{ session('alerta.texto') }}</span></label>",
...
@if(session("alerta_confirmar"))
    <script>
        Swal.fire({
            ...
            title: "<label class='d-grid gap-3 py-2'><span class='titulos_{{ $tema }} cor_fontes_{{ $tema }} {{ session('tema') == 'escuro' ? 'border-primary' : 'border-danger' }} border-top border-bottom py-3'>{{ session('alerta_confirmar.titulo') }}</span><span class='cor_fontes_{{ $tema }} fs-5'><i class='bi bi-question-diamond-fill'></i> {{ session('alerta_confirmar.texto') }}</span></label>",
...
@if(session("alerta_confirmar_render"))
    <script>
        Swal.fire({
            ...
            title: "<label class='d-grid gap-3 py-2'><span class='titulos_{{ $tema }} cor_fontes_{{ $tema }} {{ session('tema') == 'escuro' ? 'border-primary' : 'border-danger' }} border-top border-bottom py-3'>{{ session('alerta_confirmar_render.titulo') }}</span><span class='cor_fontes_{{ $tema }} fs-5'><i class='bi bi-question-diamond-fill'></i> {{ session('alerta_confirmar_render.texto') }}</span></label>",
...
@if(session("alerta_batalha"))
    <script>
        ...
        Swal.fire({
            ...
            title: "<label class='d-grid gap-3 py-2'><span class='titulos_{{ $tema }} cor_fontes_{{ $tema }} {{ session('tema') == 'escuro' ? 'border-primary' : 'border-danger' }} border-top border-bottom py-3'>{{ session('alerta_batalha.titulo') }}</span><span class='cor_fontes_{{ $tema }} fs-5'><i class='bi {{ session('alerta_batalha.icone') }}'></i> {{ session('alerta_batalha.texto') }}</span></label>",
            footer: footer,
            showClass: {
                popup: `
                    animate__animated
...
@if(session("alerta_nivel"))
    <script>
        Swal.fire({
            ...
            title: "<label class='d-grid gap-3 py-2'><span class='titulos_{{ $tema }} cor_fontes_{{ $tema }} {{ session('tema') == 'escuro' ? 'border-primary' : 'border-danger' }} border-top border-bottom py-3'>{{ session('alerta_nivel.titulo') }}</span><span class='cor_fontes_{{ $tema }} fs-5'><i class='bi bi-arrow-up-square-fill'></i> {{ session('alerta_nivel.texto') }}</span></label>",
