        ...
        <div class="fundos_card_{{ $temas[4] }} card p-3">
            <div class="d-grid gap-3 w-100">
                <div class="sombras animate__animated animate__zoomInLeft card bg-{{ $temas[0] }} border-{{ $temas[1] }} p-3">
                    <form action="{{ route("confirmar.cadastrar") }}" method="post" enctype="multipart/form-data" novalidate>
                        @csrf
                        ...
                        <div class="card-header border border-2 border-{{ $temas[1] }} text-center rounded-top">
                            ...
                            <Label class="titulos_{{ $temas[4] }} cor_fontes_{{ $temas[4] }} fw-bold fs-5">Novo Player</Label>
                        </div>

                        <div class="card-body border-start border-2 border-end border-{{ $temas[1] }}">
                            <label class="form-label cor_fontes_{{ $temas[4] }}">Usuário:</label>
                            <div class="mb-3">
                                <div class="input-group">
                                    <span class="input-group-text cor_fontes_{{ $temas[4] }} bg-{{ $temas[2] }} border-{{ $temas[1] }}">
                                        <i class="bi bi-person-fill"></i>
                                    </span>
                                    <input type="text" id="novo_usuario" class="form-control cursor bg-{{ $temas[2] }} border-{{ $temas[1] }} focus-ring focus-ring-{{ $temas[1] }} text-{{ $temas[3] }}" name="novo_usuario" placeholder="Usuario123" aria-label="Usuario123" aria-describedby="NovoUsuario" value="{{ old("novo_usuario") }}">
                                </div>
                                @error("novo_usuario")
                            ...
                            <label class="form-label cor_fontes_{{ $temas[4] }}">Email:</label>
                            <div class="mb-3">
                                <div class="input-group">
                                    <span class="input-group-text cor_fontes_{{ $temas[4] }} bg-{{ $temas[2] }} border-{{ $temas[1] }}">
                                        <i class="bi bi-envelope-at-fill"></i>
                                    </span>
                                    <input type="email" id="novo_email" class="form-control cursor bg-{{ $temas[2] }} border-{{ $temas[1] }} focus-ring focus-ring-{{ $temas[1] }} text-{{ $temas[3] }}" name="novo_email" placeholder="usuario@gmail.com" aria-label="usuario@gmail.com" aria-describedby="NovoEmail" value="{{ old("novo_email") }}">
                                </div>
                                @error("novo_email")
                                    <div class="alert alert-danger animate__animated animate__shakeX bg-danger mt-1 mb-0" role="alert">
                                        <i class="bi bi-info-circle-fill me-3"></i>{{ $message }}
                            ...
                            <div class="d-flex gap-3">
                                <div class="horizontal_vertical gap-3">
                                    <div>
                                        <label class="form-label cor_fontes_{{ $temas[4] }}">Classe:</label>
                                        <div class="input-group">
                                            <span class="input-group-text cor_fontes_{{ $temas[4] }} bg-{{ $temas[2] }} border-{{ $temas[1] }}">
                                                <i class="bi bi-sort-down"></i>
                                            </span>
                                    ...
                                    <div>
                                        <label class="form-label cor_fontes_{{ $temas[4] }}">Foto (Opcional):</label>
                                        <div class="d-grid">
                                            <div class="input-group">
                                                <span class="input-group-text cor_fontes_{{ $temas[4] }} bg-{{ $temas[2] }} border-{{ $temas[1] }}">
                                    ...
                                    <div class="d-grid justify-content-center">
                                        ...
                                        <div>
                                            <img src="{{ asset("photos/vazio.png") }}" alt="Foto Preview" id="foto_preview" class="perfil_cadastro border border-3 rounded-circle">
                                        </div>
                                    </div>
                                </div>    
                            </div>    
