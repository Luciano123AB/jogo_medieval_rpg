            ...
            <div class="row row-cols-1 row-cols-md-2 g-3">
                <div class="col cards">
                    <div class="sombras tabelas card animate__animated animate__backInLeft {{ session("tema") == "escuro" ? "bg-secondary border-primary" : "bg-dark border-danger" }} p-3">
                        <div class="d-flex justify-content-between mb-2">
                            <h4 class="cor_fontes_{{ $tema }} titulos_{{ $tema }}">-Vitórias:</h4>
                            <div></div>
                        ...
                        <div class="tabelas-scroll">
                            <table class="border border-2 {{ session("tema") == "escuro" ? "border-primary" : "border-danger" }} w-100">
                                <thead class="text-center">
                                    <th class="titulos_{{ $tema }} cor_fontes_{{ $tema }} {{ session("tema") == "escuro" ? "border-primary" : "border-danger" }} border px-1">
                                        <i class="bi bi-list-ol"></i>
                                        Nº
                                    </th>
                                    <th class="{{ session("tema") == "escuro" ? "border-primary" : "border-danger" }} border">👑</th>
                                    <th class="titulos_{{ $tema }} cor_fontes_{{ $tema }} {{ session("tema") == "escuro" ? "border-primary" : "border-danger" }} border">Você</th>
                                ...
                                <tbody>
                                    @forelse ($batalhas_vitorias as $vitorias)
                                        <tr>
                                            ...
                                            <td class="cor_fontes_{{ $tema }} {{ session("tema") == "escuro" ? "border-primary" : "border-danger" }} border text-center px-1">{{ $vitorias->ganhou }}</td>
                                            <td class="{{ session("tema") == "escuro" ? "border-primary" : "border-danger" }} border text-center">🆚</td>
                                            <td class="cor_fontes_{{ $tema }} {{ session("tema") == "escuro" ? "border-primary" : "border-danger" }} border text-center px-1">{{ $vitorias->perdeu }}</td>
                                            <td class="{{ session("tema") == "escuro" ? "border-primary" : "border-danger" }} border text-danger text-center fw-bold px-1">DERROTA</td>
                                        </tr>
                                    @empty
                                        <tr class="text-center">
                                            <td colspan="6" class="cor_fontes_{{ $tema }}">NENHUMA VITÓRIA EXISTENTE AINDA</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                ...
                <div class="col cards">
                    <div class="sombras tabelas card animate__animated animate__backInRight {{ session("tema") == "escuro" ? "bg-secondary border-primary" : "bg-dark border-danger" }} p-3">
                        <div class="d-flex justify-content-between mb-2">
                            <h4 class="cor_fontes_{{ $tema }} titulos_{{ $tema }}">-Derrotas:</h4>
                            <div></div>
                                ...
                                <tbody>
                                    @forelse ($batalhas_derrotas as $derrotas)
                                        <tr>
                                            <td class="cor_fontes_{{ $tema }} {{ session("tema") == "escuro" ? "border-primary" : "border-danger" }} border text-center fw-bold">{{ $loop->index + 1 }}</td>
                                            <td class="{{ session("tema") == "escuro" ? "border-primary" : "border-danger" }} border text-danger text-center fw-bold px-1">DERROTA</td>
                                            <td class="cor_fontes_{{ $tema }} {{ session("tema") == "escuro" ? "border-primary" : "border-danger" }} border text-center px-1">{{ $derrotas->perdeu }}</td>
                                            <td class="{{ session("tema") == "escuro" ? "border-primary" : "border-danger" }} border text-center">🆚</td>
                                            <td class="cor_fontes_{{ $tema }} {{ session("tema") == "escuro" ? "border-primary" : "border-danger" }} border text-center px-1">{{ $derrotas->ganhou }}</td>
                                            <td class="{{ session("tema") == "escuro" ? "border-primary" : "border-danger" }} border text-success text-center fw-bold px-1">VITÓRIA</td>
                                        </tr>
                                    @empty
                                        <tr class="text-center">
                                            <td colspan="6" class="cor_fontes_{{ $tema }}">NENHUMA DERROTA EXISTENTE AINDA</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
